...
const CLI_TOOLS: readonly Tool[] = [
  {
    ...
    description:
      'List the current Magic Box-managed CLI inventory. This does not inspect the user’s system PATH: a command reported as unavailable may still exist there, so check with `command -v <name>` before installing another copy. The result is read live from bundled binaries, mise installs, custom definitions, and Code CLI presets.',
  ...
  },
  {
    ...
    description:
      'Install a reusable CLI into Magic Box’s isolated mise environment. Use a name/recipe returned by cli_search, or exact executable and mise recipe derived from trusted public documentation. The operation requires user approval and accepts the same backends as BinaryManager; validation errors explain how to correct the recipe.',
