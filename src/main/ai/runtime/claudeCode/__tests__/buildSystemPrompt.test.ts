...
describe('buildSystemPrompt — report_artifacts prompt', () => {
  ...
  it('does not append it for the Magic Assistant (parity with feat/chat-page)', async () => {
    const agent = makeAgent({
      instructions: 'Assistant instructions.',
      configuration: { builtin_role: 'assistant' } as never
    })
...
describe('buildSystemPrompt — bundled-runtime guidance', () => {
  ...
  it('does not inject the runtime block for the Magic Assistant (it carries its own environment)', async () => {
    const agent = makeAgent({
      instructions: 'Assistant instructions.',
      configuration: { builtin_role: 'assistant' } as never
    })
...
describe('buildSystemPrompt — builtin Magic Assistant definition', () => {
  ...
  it('uses a minimal role fallback when the bundled template is missing and DB instructions are empty', async () => {
    ...
    expect(result as string).toContain('You are Magic Assistant, the built-in helper for Magic Box')
  })
