...
const renderPanel = (size = 1024, filePath = '/tmp/workspace/book.xlsx' as AbsoluteFilePath, fileName = 'book.xlsx') =>
  render(<SpreadsheetFilePreview filePath={filePath} fileName={fileName} metadata={{ size }} refreshKey={0} />)
...
describe('SpreadsheetFilePreview', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(window, 'api', {
      configurable: true,
      value: { fs: { readText: mocks.fsReadText } }
    })
  })
  ...
  it('renders delimited text files without invoking the xlsx workbook hook', async () => {
    renderPanel(1024, '/tmp/workspace/data.csv' as AbsoluteFilePath, 'data.csv')

    expect(await screen.findByRole('table', { name: 'data.csv' })).toBeInTheDocument()
    expect(mocks.fsReadText).toHaveBeenCalledWith('/tmp/workspace/data.csv')
    expect(mocks.useXlsxWorkbookCalls).toEqual([])
